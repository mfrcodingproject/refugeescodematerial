...
Since React is used for Single Page Applications we will need to render different components depending on the current route to simulate the navigation.

## Goals


-  [ ] Understand React routing
-  [ ] Routing your React components
...
* If the URL is `/`, your app shows the Home page.
* If the URL is `/about`, your app shows the About page.
* If the URL is `/contact`, your app shows the Contact page.
...
In this route, `:id` is a placeholder for the ID of the blog post. When a user goes to a URL like `/post/1`, the `BlogPost` component is shown, and it can access the ID (With value `"1"` in this case) from the route parameters to load the correct blog post.
...
// Now you can use postId to load the correct book from DB
}
...
Remember, to use these components you need to be inside a component that is wrapped in the `<BrowserRouter>` (or `<Router>`) component from `react-router-dom`. This is usually done at the top level of your application.
