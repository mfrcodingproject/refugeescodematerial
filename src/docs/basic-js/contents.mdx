...
In programming, **Loops** are used to repeat a block of code (iteration). For example, you can use them to run the same code over and over again, each time with a different value, or if you want to show a message 100 times, then you can use a loop. It's just some examples; you can achieve much more with loops. The most important loops are: `for`, `while`, `do...while`. You can read more information in this 📖 article [Loops and iteration](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Loops_and_iteration)
